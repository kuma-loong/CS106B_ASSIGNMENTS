...
bool hasPerfectMatchingRec(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching, const Set<Pair>& sofar, int num) {
    if (num == 0) {
        matching = sofar;
        return true;
    } else {
        string person = possibleLinks.firstKey();
        Set<string> partners = possibleLinks[person];

        Map<string, Set<string>> remainings = possibleLinks;
        remainings.remove(remainings.firstKey());
        for (Pair temp : sofar) {
            partners.remove(temp.first());
            partners.remove(temp.second());
        }

        for (string to_choose : partners) {
            Pair option(person, to_choose);
            Map<string, Set<string>> remainings_2 = remainings;
            remainings_2.remove(to_choose);
            if (hasPerfectMatchingRec(remainings_2, matching, sofar + option, num - 1)) {
                return true;
            }
        }
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    return hasPerfectMatchingRec(possibleLinks, matching, sofar, num/2);
}

bool NothingLeft (const Map<string, Map<string, int>>& possibleLinks){
    if (possibleLinks.isEmpty()) {
        return true;
    }
    for (string person : possibleLinks) {
        for (string to_link : possibleLinks[person]) {
            if (possibleLinks.containsKey(to_link)) {
                return false;
            }
        }
    }
...
void maximumWeightMatchinRec(const Map<string, Map<string, int>>& possibleLinks,const Set<Pair>& sofar, int totalWeight, Map<int, Set<Pair>>& allOptions) {
    if (NothingLeft(possibleLinks)) {
        allOptions[totalWeight] = sofar;
    } else {
        string person = possibleLinks.firstKey();
        Map<string, int> links = possibleLinks[person];
        Map<string, Map<string, int>> possibleLinks_0 = possibleLinks;
        possibleLinks_0.remove(person);

        for (Pair temp : sofar) {
            links.remove(temp.first());
            links.remove(temp.second());
        }

        maximumWeightMatchinRec(possibleLinks_0, sofar, totalWeight, allOptions);

        for (string to_link : links) {
            if (links[to_link] > 0) {
                Pair option(person, to_link);
                Map<string, Map<string, int>>possibleLinks_1 = possibleLinks_0;
                possibleLinks_1.remove(to_link);
                maximumWeightMatchinRec(possibleLinks_1, sofar + option, totalWeight + links[to_link], allOptions);
            }
        }
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    Map<int, Set<Pair>> allOptions = {};
    Set<Pair> sofar = {};
    int totalWeight = 0;

    maximumWeightMatchinRec(possibleLinks, sofar, totalWeight, allOptions);
    if (allOptions.isEmpty()) {
        return {};
    } else return allOptions[allOptions.lastKey()];
}
