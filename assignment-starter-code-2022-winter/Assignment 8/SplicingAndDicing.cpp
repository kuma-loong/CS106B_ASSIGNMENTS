...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    Nucleotide *p1 = dna, *temp_p1 = nullptr, *p2 = target;
    int count = 0;
    if(dna == nullptr) return nullptr;
    else if(target ==nullptr) return dna;
    while(p1 != nullptr) {
        temp_p1 = p1;
        p2 = target;
        while(1){
            if (temp_p1 == nullptr){
                return nullptr;
            } else if(temp_p1->value == p2->value && p2->next != nullptr) {
                p2 = p2->next;
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    Nucleotide* toSplice = findFirst(dna, target);
    Nucleotide* p = toSplice;
    if (target == nullptr) return true;
    else if (toSplice == nullptr) return false;
    else if (toSplice->prev == nullptr) {
        while(target != nullptr){
            target = target->next;
            Nucleotide* next = p->next;
            delete p;
            p = next;
        }
