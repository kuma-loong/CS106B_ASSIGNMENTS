...
bool LinearProbingHashTable::insert(const string& elem) {
    if(contains(elem) || logicalSize == allocatedSize) return false;
    int hashCode = hashFn(elem);
    int count = 0, hashPos;
    while(count < allocatedSize) {
        hashPos = hashCode % allocatedSize;
        if(elems[hashPos].type == SlotType::EMPTY || elems[hashPos].type == SlotType::TOMBSTONE) {
            elems[hashPos].value = elem;
            elems[hashPos].type = SlotType::FILLED;
            logicalSize += 1;
            return true;
        }
        hashCode++;
...
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    int hashCode = hashFn(elem);
    int count = 0;
    while(count < allocatedSize && elems[hashCode%allocatedSize].type != SlotType::EMPTY) {
        if (elems[hashCode%allocatedSize].value == elem && elems[hashCode%allocatedSize].type != SlotType::TOMBSTONE) {
            return true;
        }
        hashCode++;
        count++;
...
bool LinearProbingHashTable::remove(const string& elem) {
    if(!contains(elem) || isEmpty()) return false;
    int hashCode = hashFn(elem);
    int count = 0;
    while(count < allocatedSize && elems[hashCode%allocatedSize].type != SlotType::EMPTY) {
        if(elems[hashCode%allocatedSize].value == elem && elems[hashCode%allocatedSize].type != SlotType::TOMBSTONE){
            elems[hashCode%allocatedSize].type = SlotType::TOMBSTONE;
            logicalSize--;
            return true;
        }
        count++;
