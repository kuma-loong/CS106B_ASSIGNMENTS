...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    if (numToKeep < 0 ){
        ...
        for (string key :source){
            toSort.enqueue(key, source[key]);
        }
        for (int i = 0; i + numToKeep < source.size(); i++){
            toSort.dequeue();
        }
        for (int i = 0; i < numToKeep; i++){
            string temp = toSort.dequeue();
            topKGramsIn[temp] = source[temp];
        }
        return topKGramsIn;
    }
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    if (corpora.isEmpty()){
        ...
        for(Corpus elem : corpora){
            double key = cosineSimilarityOf(textProfile, elem.profile);
            results[key] = elem.name;
        }
        double firkey = results.lastKey();
        return results[firkey];
