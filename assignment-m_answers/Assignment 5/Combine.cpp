...
Vector<DataPoint> merge(Vector<DataPoint> left, Vector<DataPoint> right) {
    ...
    while(lh < left.size() && rh < right.size()) {
        if (left[lh].weight <= right[rh].weight) {
            result += left[lh];
            lh++;
        } else {
            result += right[rh];
    ...
    if (lh < left.size()) {
        for(int i = lh; i < left.size(); i++) {
            result += left[i];
            n++;
        }
    }
    if (rh < right.size()) {
        for(int i = rh; i < right.size(); i++) {
            result += right[i];
            n++;
        }
    }
...
Vector<DataPoint> combineRec(const Vector<Vector<DataPoint>>& sequences) {
    if (sequences.isEmpty()) {
        ...
        Vector<DataPoint> left = combineRec(sequences.subList(0, size/2));
        Vector<DataPoint> right = combineRec(sequences.subList(size/2));
        return merge(left, right);
    }
}
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    return combineRec(sequences);
}
