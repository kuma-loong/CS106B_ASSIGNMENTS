...
bool canBeMadeDisasterReadyRec(const Map<string, Set<string>>& roadNetwork,
    ...
    if (strangers.isEmpty()) {
        ...
        string newcity = strangers.first();
        Set<string> remanings = strangers - newcity;
        Set<string> citiesToSupply = {newcity};
        for (string city : roadNetwork[newcity]) {
            citiesToSupply += city;
        }

        for (string toSupply : citiesToSupply){
            if(!supplyLocations.contains(toSupply)) {
                Set<string> remanings = strangers - toSupply;
                for (string alreadyFound : roadNetwork[toSupply]) {
                    remanings -= alreadyFound;
                }

                Set<string> sofar = supplyLocations + toSupply;
                if (canBeMadeDisasterReadyRec(roadNetwork, remanings, numCities-1, sofar)){
                    supplyLocations += sofar;
                    return true;
                }
            }
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    Set<string> strangers = {};
    for (string key : roadNetwork) {
        strangers += key;
    }
    return canBeMadeDisasterReadyRec(roadNetwork, strangers, numCities, supplyLocations);
}
