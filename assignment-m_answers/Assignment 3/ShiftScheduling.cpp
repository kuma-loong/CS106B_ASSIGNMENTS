 ...
 */
void highestValueScheduleForRec(Set<Shift> shifts,const int& maxHours, Set<Shift> alreadychoose, Map<int, Set<Shift>>& allchoice) {
    if (shifts.isEmpty()) {
        int sumOfvalue = 0;
        for (Shift elme : alreadychoose) {
            sumOfvalue += valueOf(elme);
        }
        if (!allchoice.isEmpty() && allchoice.firstKey() <= sumOfvalue){
            allchoice[sumOfvalue] = alreadychoose;

        } else allchoice[sumOfvalue] = alreadychoose;


        return;
    } else {
        Shift temp = shifts.first();
        shifts.remove(temp);
        int hours = 0;
        bool overlap = false;
        for (Shift elme : alreadychoose) {
            hours += lengthOf(elme);
            if (overlapsWith(elme, temp)) {
                overlap = true;
            }
        }
        if (overlap) {
            highestValueScheduleForRec(shifts, maxHours, alreadychoose, allchoice);
        } else if (hours + lengthOf(temp) > maxHours) {
            highestValueScheduleForRec(shifts, maxHours, alreadychoose, allchoice);
        } else {
            highestValueScheduleForRec(shifts, maxHours, alreadychoose + temp, allchoice);
            highestValueScheduleForRec(shifts, maxHours, alreadychoose, allchoice);
        }
    }
}
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    Set<Shift> shifts_1 = shifts;
    Set<Shift> alreadychoose = {};
    Map<int, Set<Shift>> allchoice;
    highestValueScheduleForRec(shifts_1, maxHours, alreadychoose, allchoice);
    //Set<Shift> highestValue = allchoice.get(allchoice.lastKey());
    return allchoice[allchoice.lastKey()];
}
