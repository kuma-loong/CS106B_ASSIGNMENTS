 ...
 */
void allEmphasesOfRec(Vector<string> allunits, Set<string>& allEmphases, string result) {
    if (allunits.isEmpty()) {
        allEmphases += result;
    } else {
        string elme = allunits[0];
        allunits = allunits.subList(1, allunits.size()-1);
        if(elme[0] < 65 || elme[0] >122) {
            allEmphasesOfRec(allunits, allEmphases, result + elme);
        } else {
            allEmphasesOfRec(allunits, allEmphases, result + elme);
            elme = toUpperCase(elme);
            allEmphasesOfRec(allunits, allEmphases, result + elme);
        }
    }
}
Set<string> allEmphasesOf(const string& sentence) {
    ...
    Vector<string> allunits = tokenize(sentence);
    for (int i = 0; i < allunits.size(); i++) {
        allunits[i] = toLowerCase(allunits[i]);
    }
    string result = "";
    allEmphasesOfRec(allunits, allEmphases, result);
    return allEmphases;
}
