...
void HeapPQueue::enqueue(const DataPoint& data) {
    if(logicalSize == allocatedSize - 1) {
        DataPoint* helper = new DataPoint[allocatedSize*2];
...
DataPoint HeapPQueue::dequeue() {
    if(logicalSize == 0){
        ...
        DataPoint to_dequeue = elems[logicalSize];
        logicalSize--;
        int i = 1;
        while(i <= logicalSize/2) {
            if(elems[i].weight > elems[i*2+1].weight && i*2+1 <= logicalSize && elems[i*2+1].weight <= elems[i*2].weight){
                swap(elems[i], elems[i*2+1]);
                i = i*2+1;
            }else if(elems[i].weight > elems[i*2].weight && i*2 <= logicalSize){
                swap(elems[i], elems[i*2]);
                i *= 2;
            } else break;
        }
