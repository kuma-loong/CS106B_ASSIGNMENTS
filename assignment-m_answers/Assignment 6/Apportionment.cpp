...
Map<string, int> apportion(const Map<string, int>& populations, int numSeats) {
    if(populations.size() > numSeats){
        error("Seats isnot enough");
    } else {
        HeapPQueue pq;
        Map<string, int> apport;
        int numStates = populations.size();
        ...
        numSeats -= numStates;
        while(numSeats > 0){
            DataPoint highest = pq.dequeue();
            apport[highest.name]++;
            highest.weight = -populations[highest.name] / sqrt(apport[highest.name] * (apport[highest.name]+1));
            pq.enqueue(highest);
            numSeats--;
        }
        return apport;
