...
RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    this->hashFn = hashFn;
    numsSlots = hashFn.numSlots();
    logicalSize = 0;
    elems = new Slot[numsSlots];
    for (int i = 0; i < numsSlots; i++) {
        elems[i].value = "";
...
bool RobinHoodHashTable::insert(const string& elem) {
    if(logicalSize == numsSlots || contains(elem)) return false;
    int hashCode = hashFn(elem), count = 0;
    while (elems[hashCode%numsSlots].distance != EMPTY_SLOT
           && count < numsSlots) {
        if (count > elems[hashCode%numsSlots].distance) {
            string tempValue = elems[hashCode%numsSlots].value;
            int tempDistance = elems[hashCode%numsSlots].distance;
            elems[hashCode%numsSlots].value = elem;
            elems[hashCode%numsSlots].distance = count;
            int i = 1;
            hashCode++;
            while(elems[(hashCode)%numsSlots].distance != EMPTY_SLOT) {
                if ((tempDistance+i) > elems[(hashCode)%numsSlots].distance) {
                    int temp = elems[(hashCode)%numsSlots].distance;
                    elems[(hashCode)%numsSlots].distance = tempDistance+i;
                    tempDistance = temp;
                    swap(tempValue, elems[(hashCode)%numsSlots].value);
                    i = 0;
                }
                hashCode++;
                i++;
            }
            if (elems[(hashCode)%numsSlots].distance == EMPTY_SLOT) {
                elems[(hashCode)%numsSlots].distance = tempDistance + i;
                elems[(hashCode)%numsSlots].value = tempValue;
            }
            logicalSize++;
            return true;
        }
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    int hashCode = hashFn(elem), count = 0;
    while (elems[hashCode%numsSlots].distance != EMPTY_SLOT
            && count <= elems[hashCode%numsSlots].distance
            && count < numsSlots) {
        if (elems[hashCode%numsSlots].value == elem) {
            return true;
        }
        hashCode++;
        count++;
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    while (elems[hashCode%numsSlots].distance != EMPTY_SLOT
           && count < logicalSize
           && count <= elems[hashCode%numsSlots].distance) {
        if (elems[hashCode%numsSlots].value == elem) break;
        hashCode++;
        count++;
    }
    if (elems[hashCode%numsSlots].value == elem && elems[hashCode%numsSlots].distance != EMPTY_SLOT) {
        logicalSize--;
        elems[hashCode%numsSlots].distance = EMPTY_SLOT;
        while (elems[(hashCode + 1)%numsSlots].distance != EMPTY_SLOT && elems[(hashCode + 1)%numsSlots].distance != 0) {
            elems[(hashCode+1)%numsSlots].distance -= 1;
            elems[hashCode%numsSlots].distance = EMPTY_SLOT;
            swap(elems[hashCode%numsSlots], elems[(hashCode+1)%numsSlots]);
            hashCode++;
        }
        return true;
    }
